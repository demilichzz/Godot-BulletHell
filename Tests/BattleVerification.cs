...
public partial class BattleVerification : Node
{
	...
	private void VerifyOriginal()
	{
		...
		// 独立子弹保持旧接口默认倍率和到期释放行为。
		var sample = new Bullet();
		sample.Initialize(new Vector2(10, 20), 90, 100, 0.25f);
		container.AddChild(sample);
		Check(sample.GetNode<Sprite2D>("Sprite").Scale == Vector2.One * 2, "旧接口默认倍率2");
		sample._PhysicsProcess(0.1);
		Check(sample.Position.DistanceTo(new Vector2(10, 30)) < 0.001, "旧接口运动");
		sample._PhysicsProcess(0.15);
		Check(sample.IsQueuedForDeletion(), "寿命释放");
		// 枚举图集颜色，确认帧映射保持原样。
		for (int color = 0; color < 10; color++)
		{
			var colored = new Bullet();
			colored.Initialize(Vector2.Zero, 180, 100, 4, color, 1);
			container.AddChild(colored);
			Check(colored.GetNode<Sprite2D>("Sprite").Frame == color, "图集颜色");
		}
		// 无效颜色与倍率必须保持抛错。
		var invalid = new Bullet();
		container.AddChild(invalid);
		try { invalid.Initialize(Vector2.Zero, 0, 1, 1, 10); Check(false, "颜色未校验"); }
		catch (ArgumentOutOfRangeException) { _checks++; }
		try { invalid.Initialize(Vector2.Zero, 0, 1, 1, 0, float.NaN); Check(false, "倍率未校验"); }
		catch (ArgumentOutOfRangeException) { _checks++; }
		invalid.Free();
		world.Free();
	}
